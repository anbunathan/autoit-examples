...
; List all the files and folders in the desktop directory using the default parameters.
 Global $aFileList = _FileListToArray("D:\Webapp\dayananda\BTech\Agile\Attendance", "*csv")
 If @error = 1 Then
 ...
 For $fileIndex = 1 to $aFileList[0]
	Global $hFileOut = FileOpen('D:\Webapp\dayananda\BTech\Agile\consolidated\attendance.csv', 128)
   ...
   EndIf
	...
	$filename = 'D:\Webapp\dayananda\BTech\Agile\Attendance\' & $aFileList[$fileIndex]
   ...
   $date = StringRegExpReplace($filenameWext, '(dsu-btech_agile )*[\(]*(\).csv)*', '')
   ...
   Next
   Local  $aRetArray
   Local $fileName = 'D:\Webapp\dayananda\BTech\Agile\consolidated\attendance.csv'
